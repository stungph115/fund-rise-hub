...
function ListCard({ client, selectedCard, setSelectedCard }) {
    ...
    async function getPaymentMethods(client) {
        ...
        axios.get(env.URL + 'card-payment/' + client.id).then((res) => {
    ...
    async function setDefaultCard(paymentMethod) {
        setIsLoading(true)
        axios.post(env.URL + 'card-payment/default', {
    ...
    async function deleteCard(paymentMethod) {
        setIsLoading(true)
        axios.delete(env.URL + 'card-payment/' + paymentMethod).then((res) => {
            /* console.log(res) */
            getPaymentMethods(client)
            setIsLoading(false)
            ...
            <Fade >
                <div style={{ width: 'fit-content' }}>
                    {cards.length > 0 ?
                            ...
                            {cards.map((item, i) => {
                                ...
                                return (
                                    <React.Fragment key={i}>
                                        <div className={selectedCard && selectedCard.id === item.id ? 'credit-card-item choosen' : 'credit-card-item'} style={{ width: 500 }} onClick={() => setSelectedCard(item)}>
                                            <FontAwesomeIcon icon={getIconByBrand(item.display_brand)} style={{ width: '10%' }} size='xl' className='card-brand' />
                                            <div className='card-last4' style={{ width: '40%', textAlign: 'center' }}>**** **** **** {item.last4}</div>
                                            <div className='card-exp' style={{ width: '10%' }}>{item.exp_month > 9 ? item.exp_month : '0' + item.exp_month}/{String(item.exp_year).slice(-2)}</div>
                                            {item.id === cardDefault ?
                                                <div style={{ paddingInline: 10, width: '40%', textAlign: 'center' }}>défaut</div>
                                                :
                                                <div className="set-default-text" style={{ paddingInline: 10, width: '40%', textAlign: 'center' }} onClick={() => setDefaultCard(item.id)}>
                                                    {isLoading ?
                        ...
                        :
                        <div className='empty-text' style={{ width: '100%' }}>Aucune méthode de paiement enregistrée</div>
                    }
